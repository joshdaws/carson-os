...
- **[P0] Carson's tools work again — every tool call was silently failing since v0.5.7.** v0.5.7's AsyncLocalStorage refactor fixed a real cross-tenant bug (#63) but called `query()` *outside* the `toolContextStorage.run(...)` frame. Node's `AsyncLocalStorage` only propagates context to async resources created *inside* an active `run()` frame, and the Claude Agent SDK wires up its subprocess stdout listeners and MCP transport callbacks during the synchronous part of `query()`. Those listeners snapshotted an empty ALS context. When the SDK later fired a tool callback through them, `runCachedTool` saw `toolContextStorage.getStore() === undefined` and threw `Tool '${name}' invoked outside of execute() context`. The MCP layer marshalled that as a tool error result, the model gave up tools, and Carson silently fell back to text-only replies. Production logs showed **0 tool calls across 25 conversations** post-deploy — including the Telegram thread where Josh sent X URLs and Carson confabulated `"the X tools require something called toolContextStorage that isn't being initialized"` instead of fetching the post. Fix moves `query()` and `triggerRefresh` assignment INSIDE the `toolContextStorage.run` frame so the SDK's event listeners snapshot the right per-request `toolContextRef`. Empirically verified: after restart, Carson made her first successful tool call (xAI Grok live X-search) within seconds. The cross-tenant isolation contract from #63 is unchanged — each `execute()` still has its own `toolContextRef`. New regression tests in `subprocess-adapter-sdk-transport.test.ts` install a faithful mock SDK (setImmediate-based tool dispatch) and assert the executor is invoked for both single and concurrent `execute()` calls. Both tests were verified to fail on the v0.5.7 buggy code by reverting and re-running. The pre-v0.5.7 isolation tests in `subprocess-adapter-tool-isolation.test.ts` still pass — they pin the closure's behavior in isolation but never exercised the SDK transport path that was actually broken.

- **`ProcessTransport is not ready for writing` log spam silenced.** `triggerRefresh` is fire-and-forget from `onCall` — a TOOL_LIST_MODIFYING tool result enqueues an MCP refresh while the SDK is shutting down, racing with subprocess teardown. v0.5.7 logged 222 of these per conversation in stderr. Fix adds a `pumpDone` flag, set in a `try/finally` around the for-await loop. `triggerRefresh` now bails early if the pump has exited, and the `catch` block additionally checks `pumpDone` before warning so a tear-down `setMcpServers` failure doesn't surface as a spurious error.

- **Compilation agent stops retrying entities that can never compile.** `compileEntity` threw plain `Error` for "Entity not found" and "Type X is not compilable", which the catch block recorded as `last_error` but left `dirty_at` set — the row stayed in the dirty queue forever and produced `[compilation-agent] tick: failed=2` every 60 seconds in production for the past two days. Fix adds a typed `NonCompilableEntityError`. The catch block: (a) always records `last_error` for forensics, then (b) for `NonCompilableEntityError` only, clears `dirty_at` *with CAS* so a `markDirty` that fired during the failed compile (e.g., entity was momentarily missing on disk and just got recreated) survives. Adversarial review caught that the first-pass fix was wiping `dirty_at` by id only — the CAS guard preserves the success-path contract. After deploy, the two stuck rows in the family's database (`2026-05-01-grant-act-daily-prep-20-30-minutes` collection=grant, `2026-05-02-layrkits-sprite-sheet-pipeline` collection=josh) self-healed on the first post-restart tick.

- **Wikilinks with underscores now resolve to canonical entities.** Memory file slugs are kebab-case on disk (`user-josh.md`, `becca-daws.md`) but agents kept emitting `[[user_josh]]` in conversation memories — partly because Claude Code's auto-memory uses underscored filenames. The slug grammar rejected underscores entirely, so 222 valid-intent wikilinks per log-window dropped on the floor as "malformed slug". Fix removes `_` from `SLUG_FORBIDDEN` and normalizes underscores to hyphens at parse time, then collapses runs of hyphens and trims leading/trailing hyphens so adversarial inputs like `[[foo__bar]]`, `[[__foo]]`, `[[foo_]]` produce clean kebab (`foo-bar`, `foo`, `foo`) instead of invalid `--`-bearing slugs. `parseWikilinks` now returns both the canonical `slug` (kebab) and the `rawSlug` (as it appeared in the body) — `inferLinkType` uses the raw form to locate the wikilink in the original text so the `parent`/`spouse`/`works_at` classifier still works against `[[user_josh]]` even though the persisted slug is `user-josh`. Slash, backslash, and dot still rejected.

- **`inferLinkType` no longer misclassifies wikilinks via prefix match.** `body.indexOf("[[" + slug)` matched any `[[slug-prefix...]]`, so a body like `"Josh works at [[josh]] and is married to [[josh-daws]]"` queried for slug `"josh"` returned the index of `[[josh-daws]]` and read `"is married to"` as the context for `[[josh]]`. The bug was pre-existing but the underscore normalization in this release expanded its surface (`[[user_josh]]` rawSlug coexisting with potentially-unrelated `[[user_josh_grandparent]]`). Fix terminates the match with `[[${slug}]]` or `[[${slug}|` — the only legal closers — so prefix overlap can't confuse the lookup.
