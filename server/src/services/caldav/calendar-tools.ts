/**
 * CalDAV calendar tool definitions + handler.
 *
 * Three tools (identical schemas to the Google Calendar tools so agents
 * work the same way regardless of which backend is active):
...
import type { CalDavProvider } from "./caldav-provider.js";

// ── Tool definitions ───────────────────────────────────────────────

export const CALDAV_CALENDAR_TOOLS: ToolDefinition[] = [
  {
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        calendar: {
          type: "string",
          description: "Which calendar to add to (default: first available).",
        },
      },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        calendarId: {
          type: "string",
          description: "Not required for CalDAV — the calendar is inferred from the event ID.",
        },
      },
      required: ["eventId"],
    },
...
export function createCalDavCalendarToolHandler(
    ...
    try {
      switch (name) {
        ...
        case "create_calendar_event": {
          ...
          return {
            content: `Event created: "${event.summary}" on ${event.start}`,
          };
        }
