    ...
    switch (ev.type) {
      ...
      case "item.completed": {
        ...
        if (item.type === "agent_message" && typeof item.text === "string") {
          // Codex emits each agent_message as a complete block, and a turn can
          // contain several (split by tool calls). The `content` getter joins
          // them with a blank line, so the streamed delta for every block after
