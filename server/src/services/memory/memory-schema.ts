...
export function buildMemorySchemaInstructions(schema: MemorySchema): string {
  ...
  lines.push(
    ...
    "",
    "**Background enrichment (v5).** A background worker captures atoms from your",
    "conversation turns automatically — entities mentioned, factual claims, dated",
    "events, commitments. You do NOT need to manually `save_memory` every fact",
    "during natural conversation. The worker handles routine capture. Reserve",
    "explicit `save_memory` for these specific cases:",
    "- The user explicitly asks you to remember or save something",
    "- You decide a fact is HIGH importance (≥ 7) and want it logged immediately",
    "  rather than waiting for the nightly compilation pass",
    "- You're correcting a prior error → use `correct_memory`",
    "Otherwise, trust the worker. Don't apologize that you 'couldn't save' —",
    "the worker is doing it asynchronously.",
    "",
    "Tools:",
    "- `search_memory` — search before answering AND before saving",
    "- `save_memory` — save a new memory deliberately (high-importance or explicit ask)",
    "- `update_memory` — enrich an existing memory; extend `topics` when new concepts surface",
    "- `delete_memory` — remove outdated, incorrect, or wrong memories",
    "- `correct_memory` — append a correction atom to a prior memory (importance: 10)",
    "- `read_memory` — fetch the full body of a memory you found via `search_memory`",
