...
import { z } from 'zod';
import type { AdapterType, AdapterExecuteParams, AdapterExecuteResult } from '@carsonos/shared';
...
class ClaudeCodeAdapter implements Adapter {
  ...
  async execute(params: AdapterExecuteParams): Promise<AdapterExecuteResult> {
    ...
    const userMessage = messages.map((m) => (m.role === 'user' ? m.content : `[assistant]: ${m.content}`)).join('\n\n');
    ...
    const args = ['--output-format', 'json', '--max-turns', '3', '--allowed-tools', 'none', '--system-prompt-file', promptFile, '-p'];
    ...
    return new Promise<AdapterExecuteResult>((resolve, reject) => {
      ...
      child.on('close', (code) => {
        try {
          ...
          const content = typeof parsed.result === 'string' ? parsed.result : typeof parsed.content === 'string' ? parsed.content : stdout;
...
class CodexAdapter implements Adapter {
  ...
  async execute(params: AdapterExecuteParams): Promise<AdapterExecuteResult> {
    ...
    return new Promise<AdapterExecuteResult>((resolve, reject) => {
      ...
      child.on('close', (code) => {
        if (code !== 0) {
          reject(new Error(`Codex exited with code ${code}: ${stderr || 'no stderr'}`));
          return;
        }
...
class ClaudeAgentSdkAdapter implements Adapter {
  ...
  async execute(params: AdapterExecuteParams): Promise<AdapterExecuteResult> {
    ...
    const userPrompt = messages.map((m) => (m.role === 'user' ? m.content : `[assistant]: ${m.content}`)).join('\n\n');
    ...
    if (tools && tools.length > 0 && toolExecutor) {
      const mcpTools = tools.map((t) => {
        ...
        return tool(t.name, t.description, shape, async (input: Record<string, unknown>) => {
    ...
    const sdkModel = model === 'claude-sonnet-4-6' ? 'sonnet' : model === 'claude-opus-4-6' ? 'opus' : model === 'claude-haiku-4-5-20251001' ? 'haiku' : (model ?? DEFAULT_MODEL);
    ...
    for await (const message of conversation) {
      ...
      if (message.type === 'result') {
        ...
        if (message.subtype === 'success') {
          const sdkResult = 'result' in message && typeof message.result === 'string' ? message.result : '';
          resultText = assistantTextBlocks.length > 0 ? assistantTextBlocks.join('\n\n') : sdkResult;
        } else {
          const errors: string[] = 'errors' in message && Array.isArray(message.errors) ? message.errors : [];
          resultText = errors.length > 0 ? `Error: ${errors.join('; ')}` : 'Something went wrong. Please try again.';
        }
      }
    }
    ...
    console.log(
      `[adapter] Agent SDK: ${totalMs}ms, ${numTurns} turns, ${allToolCalls.length} tool calls, resume=${isResume ? 'yes' : 'no'}, session=${capturedSessionId ?? 'none'}`,
    );
    if (totalCost != null) {
